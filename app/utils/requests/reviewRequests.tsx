...
export const getBeerReviews = async ({
	...
	const res = await fetch(
		url + `/beers/${id}/reviews${query ? `?${query}` : ""}`,
		{
			method: "GET",
			headers: {
				"Content-Type": "application/json",
...
export const getReviewConfirm = async ({
	...
	if (res.status === 404) {
		return { reviewed: false }; // normalize 404 → "not reviewed"
	}
...
export const adminReviewsGet = async ({
	token,
	limit,
	offset,
	deleted,
	...
	const res = await fetch(
		url +
			`/beers/admin/reviews?limit=${limit}&offset=${offset}&deleted=${deleted}`,
		{
			method: "GET",
			headers: {
				Authorization: `Bearer ${token}`,
...
export const hardDeleteReview = async (id: string, token: string) => {
	const res = await fetch(url + "/beers/admin/hard-delete/review/" + id, {
		method: "DELETE",
		headers: {
			Authorization: `Bearer ${token}`,
		},
...
export const undoSoftDeleteReview = async (id: string, token: string) => {
	const res = await fetch(url + "/beers/review/delete/undo/" + id, {
		method: "PUT",
		headers: {
			Authorization: `Bearer ${token}`,
		},
